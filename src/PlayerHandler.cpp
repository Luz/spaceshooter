        ...
        if(mPlayers[i] == NULL)
        {
            switch (typ)
            {
                ...
                case ALIEN:
                     ...
                     std::cout << "sorry heres something TODO in PlayerHandler.cpp" << std::endl;
                    break;
                default://Else
...
void PlayerHandler::keyDown(unsigned int key, unsigned int playernumber)
{
        switch (key)
        {
            ...
            case 5: mPlayers[playernumber]->schiessen(GREEN); break;
            case 6: mPlayers[playernumber]->schiessen(ROCKET); break;
